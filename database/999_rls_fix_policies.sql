-- RLS Helper Functions & Policy Fixes
-- Safe to re-run on existing database (uses OR REPLACE / IF EXISTS)
...
ALTER TABLE IF EXISTS module_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS status_definitions ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS kpi_definitions ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS kpi_logs ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS unit_progress ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS item_progress ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS daily_reports ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS daily_report_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS contracts ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS subcontracts ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS contract_scope_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS contract_variations ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS contract_invoices ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS contract_invoice_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS work_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS work_breakdown_structure ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS work_tasks ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS work_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS work_request_lines ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS safety_observations ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS toolbox_talks ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS ppe_issuance ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS safety_audits ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS employees ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS labor_groups ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS payroll_details ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS payroll_settings ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS suppliers ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS materials_catalog ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS goods_receipt_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS inventory_stocks ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS material_issues ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS material_issue_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS leads ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS customers ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS unit_sales ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS payment_plans ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS collections_schedule ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS handover_records ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS technical_tickets ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS ticket_comments ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS ticket_references ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS document_references ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "unit_progress_insert" ON unit_progress FOR INSERT WITH CHECK (
    is_admin() OR has_project_role((SELECT project_id FROM units WHERE id = unit_progress.unit_id), 'engineer')
);
...
CREATE POLICY "invoices_insert" ON contract_invoices FOR INSERT WITH CHECK (
    is_admin() OR has_project_role((SELECT project_id FROM contracts WHERE id = contract_invoices.contract_id), 'finance')
);
...
CREATE POLICY "wir_select" ON work_requests FOR SELECT USING (
    is_admin() OR has_project_access(project_id) OR requested_by = auth.uid() OR inspected_by = auth.uid()
);
CREATE POLICY "wir_insert" ON work_requests FOR INSERT WITH CHECK (
    is_admin() OR current_user_role() IN ('engineer','quality','main_contractor','subcontractor') OR has_project_role(project_id, 'engineer')
);
CREATE POLICY "wir_update" ON work_requests FOR UPDATE USING (
    is_admin() OR requested_by = auth.uid() OR inspected_by = auth.uid() OR has_project_role(project_id, 'quality')
);

CREATE POLICY "wir_lines_select" ON work_request_lines FOR SELECT USING (
    is_admin() OR EXISTS (
        SELECT 1 FROM work_requests wr JOIN user_projects up ON wr.project_id = up.project_id
...
CREATE POLICY "employees_select" ON employees FOR SELECT USING (
    is_admin() OR has_project_access(project_id) OR current_user_role() IN ('hr', 'finance')
);
...
CREATE POLICY "attendance_select" ON attendance FOR SELECT USING (
    is_admin() OR current_user_role() IN ('hr', 'finance') OR
...
CREATE POLICY "payroll_select" ON payroll_runs FOR SELECT USING (
    is_admin() OR current_user_role() IN ('hr', 'finance') OR has_project_access(project_id)
);
...
CREATE POLICY "pr_select" ON purchase_requests FOR SELECT USING (
    is_admin() OR has_project_access(project_id) OR requested_by = auth.uid()
);
...
CREATE POLICY "unit_sales_select" ON unit_sales FOR SELECT USING (
    is_admin() OR current_user_role() IN ('sales','finance') OR has_project_access(project_id) OR
    EXISTS (SELECT 1 FROM user_profiles WHERE id = auth.uid() AND role = 'client')
);
CREATE POLICY "unit_sales_insert" ON unit_sales FOR INSERT WITH CHECK (is_admin() OR current_user_role() = 'sales');
CREATE POLICY "unit_sales_update" ON unit_sales FOR UPDATE USING (is_admin() OR current_user_role() = 'sales');
...
CREATE POLICY "handover_select" ON handover_records FOR SELECT USING (
    is_admin() OR has_project_access((SELECT project_id FROM units WHERE id = handover_records.unit_id))
);
...
CREATE POLICY "tech_tickets_select" ON technical_tickets FOR SELECT USING (
    is_admin() OR has_project_access(project_id) OR requested_by = auth.uid() OR assigned_to = auth.uid()
);
...
CREATE POLICY "ticket_comments_select" ON ticket_comments FOR SELECT USING (
    is_admin() OR EXISTS (
        SELECT 1 FROM technical_tickets tt JOIN user_projects up ON tt.project_id = up.project_id
...
CREATE POLICY "ticket_comments_insert" ON ticket_comments FOR INSERT WITH CHECK (
    is_admin() OR EXISTS (
        ...
        WHERE tt.id = ticket_comments.ticket_id AND (tt.requested_by = auth.uid() OR tt.assigned_to = auth.uid())
    )
);
...
CREATE POLICY "documents_select" ON documents FOR SELECT USING (
    is_admin() OR has_project_access(project_id) OR (confidentiality IN ('public', 'internal'))
);
...
CREATE POLICY "doc_versions_select" ON document_versions FOR SELECT USING (
    is_admin() OR EXISTS (
        SELECT 1 FROM documents d JOIN user_projects up ON d.project_id = up.project_id
...
CREATE POLICY "doc_versions_insert" ON document_versions FOR INSERT WITH CHECK (
    is_admin() OR EXISTS (
        SELECT 1 FROM documents d WHERE d.id = document_versions.document_id AND d.uploaded_by = auth.uid()
    )
);
